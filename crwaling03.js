...
async function main() {
  ...
  for (let i = 0; i < titles.length; ++i) {
    let book = {}; // book 객체 초기화
    book.title = titles[i];
    book.writer = writers[i].replace(/ /g, "");
    book.price = prices[i].replace(/,|원/g, "");
    books.push(book); // books 배열에 book 객체 삽입(객체배열 생성)
  }

  // 생성된 도서 객체배열 확인
  console.log(books);

  // 생성된 도서 객체배열을 JSON 문자열로 변환하고
